...
const Collections = () => {
  ...
  const collectionToUpdate = collections.find(c => c.collection_number === collectionNumber);
  if (collectionToUpdate) {
    axios.get(`http://localhost:3030/schoolsDetails`)
      .then(response => {
        const schools = response.data;
        const school = schools.find(s => s.collections.some(c => c.collection_number === collectionNumber));
        if (school) {
          const updatedSchoolCollections = school.collections.map(c =>
            c.collection_number === collectionNumber ? { ...c, status: newStatus } : c
          );
          axios.patch(`http://localhost:3030/schoolsDetails/${school.id}`, {
            ...
            <tr key={collection.collection_number}>
              ...
              <td>
                <button
                  ...
                  onClick={() => handleStatusChange(collection.collection_number, 'Valid')}
                >
                  Mark as Valid
                </button>
                <button
                  className="btn btn-danger btn-sm"
                  onClick={() => handleStatusChange(collection.collection_number, 'Bounced')}
                >
                  Mark as Bounced
                </button>
              </td>
